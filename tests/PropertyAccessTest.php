...
class PropertyAccessTest extends TestCase
{
    ...
    private function assertStrategySetPropertyReturns(MockObject $strategy, $returnValue): void
    {
        $strategy
            ->expects($this->once())
    ...
    public function setPropertyValue_oneStrategyModifiesValue_notCallOtherStrategies(): void
    {
        ...
        $this->assertStrategySupportsAndReturns($this->strategy2, true);
        $this->assertStrategySetPropertyReturns($this->strategy2, $this->valueObject);
    ...
    public function setPropertyValue_noStrategyWasSuccessful_throwFailedSettingPropertyException(): void
    {
        ...
        $this->assertStrategySupportsAndReturns($this->strategy2, true);
        $this->assertStrategySetPropertyReturns($this->strategy2, null);

        $this->assertStrategySupportsAndReturns($this->strategy3, true);
        $this->assertStrategySetPropertyReturns($this->strategy3, null);

        $this->expectException(FailedSettingPropertyException::class);
        $this->propertyAccess->setPropertyValue($this->valueObject, self::PROPERTY_NAME, self::PROPERTY_VALUE);
    }
}
